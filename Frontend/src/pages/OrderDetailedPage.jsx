...
const OrderDetailedPage = () => {
  ...
  useEffect(() => {
    const mockOrderDetails = {
      ...
      shippingMethod: 'Standard', // Fixed typo here
      shippingAddress: { city: "Karachi", country: "Pakistan" },
      orderItems: [  // Fixed typo here
        {
          productId: "1",
          name: "Jacket",
          price: 120,
        ...
        (<div className='p-4 sm:p-6 rounded-lg border '>
          <div className='flex flex-col sm:flex-row justify-between mb-8'>
            ...
            <div className='flex flex-col items-start sm:items-end mt-4 sm:mt-0 gap-2'>
              ...
              <span className={`${orderDetails.isDelivered ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'} px-3 py-1 rounded-full text-sm font-medium mb-2`}>
                {orderDetails.isDelivered ? "Delieverd" : "Pending"}
              </span>
            </div>
          </div>
          <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mb-8'>
            ...
            <div>
              ...
              <p>Address:{orderDetails.shippingAddress.city},${orderDetails.shippingAddress.city}</p>
            </div>
          </div>
                ...
                {orderDetails.orderItems.map((item)=>(
                  <tr key={item.productId} className='border-b'>
                    <td className='py-2 px-4 flex items-center '>
                      <img src={item.image} alt={item.name} className='w-12 h-12 object-cover rounded-e-lg mr-4' />
          ...
          <Link to='/my-orders' className='text-blue-500 hover:underline'>Back to My Orders
          </Link>
        </div>)
      }
    </div>
